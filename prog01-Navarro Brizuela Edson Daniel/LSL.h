...
void LSL<T>::guardar(){
    ...
    if(!archivo.is_open()){
        ...
        for(size_t i(0); i < size(); i++){
            archivo << getData(i).getNombre() << "|";
            archivo << getData(i).getLiga() << "|";
            archivo << getData(i).getPais() << "*";
        }
        archivo.close();
    }
}
...
void LSL<T>::cargar(){
    ...
    if(!archivo.is_open()){
        ...
        while(true){
            ...
            getline(archivo, liga, '|');
            getline(archivo, pais, '*');
            equipo.setNombre(nombre);
            equipo.setLiga(liga);
            equipo.setPais(pais);
            this->push_back(equipo);
        }
    }
}
...
//         T equipo;
//         getline(archivo, nombre);
//         stringstream input_registro(nombre);
//         while(getline(input_registro, pais, '*')){
//             stringstream input_campo(pais);
//             cout << pais << endl;
//             do{ 
                // If para separar campos
//                 cout << liga << endl;
//             }while(getline(input_campo, liga, '|'));
//         }
//     }
// }
